...
1. `resources/prashna_standard-layer.md`：来源标签、准入、白名单；
2. `resources/prashna_question-taxonomy.md`：单问与 A／B／C 支持级；
3. `resources/prashna_house-karaka-map.md`：一个主事项宫和专题入口；
4. `resources/prashna_judgment-rubric.md`：适用规则账本和三档组合；
5. `resources/prashna_moon-policy.md`：Moon 当前事实的使用边界。

只在用户显式启用时读取：

- Tajika：`resources/prashna_tajika-optional.md`
- KP：`resources/prashna_kp-optional.md`
- 本命矛盾裁决：`resources/prashna_cross-natal-policy.md`
- 既有盘追问：`resources/prashna_qa_rules.md`

---

## 隔离硬约束

1. 只读调用共享 `scripts/engine.py`；不向共享 engine 增加
